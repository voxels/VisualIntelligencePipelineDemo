    ...
    public func process(
        ...
        if let existing {
            ...
            if let urlString = input.url, let url = URL(string: urlString), let enrichmentService {
                if url.scheme?.lowercased().hasPrefix("diver") == false {
        ...
        await withTaskGroup(of: ParallelEnrichmentResult?.self) { group in
            ...
            group.addTask {
                 ...
                 if url.scheme?.lowercased().hasPrefix("diver") == true { return nil }
            ...
            group.addTask { [service] in
                 guard isProduct, let query = productQuery else { return nil }
                 
                 // Use DDG Enrichment `enrich(query:)` which works for products
                 if let ddgService = service {
                     if let data = try? await ddgService.enrich(query: query, location: nil) {
                         // Extract better URL if available
                         var betterURL: URL?
                         if let web = data.webContext, let webUrl = web.snapshotURL, web.isReaderAvailable, let url = URL(string: webUrl) {
                              // This is a flaw in `EnrichmentData`.
                              // But wait, `LinkEnrichmentService` protocol returns `EnrichmentData`.
                              // Where is the link URL stored?
                              // `enrich(url:)` implies input URL is source.
                              // `DDG` enrich via query -> `AbstractURL` is lost if not in `EnrichmentData`!
                              // I MUST Add `url` to `EnrichmentData` if I want to retrieve it.
                         }
                         
                         // For now, just return tags
                         // For now, just return tags
                     if let productURL = betterURL {
                          processed.productSearchURL = productURL
                     }
                     
                     return ParallelEnrichmentResult(productData: data)
                 }
                 return nil
            }
            return nil
