...
final class AppGroupConfigTests: XCTestCase {
    func testDefaultIdentifiers() {
        XCTAssertEqual(AppGroupConfig.default.groupIdentifier, "group.com.secretatomics.Diver")
    ...
    func testMissingContainerThrows() {
        XCTAssertThrowsError(try AppGroupContainer.containerURL(urlProvider: { _ in nil })) { error in
            XCTAssertEqual(error.localizedDescription, "App group container unavailable for group.com.secretatomics.Diver.")
        }
    }
}
