...
//        let initialEnrichment = CompositeLinkEnrichmentService(services: [duckDuckGoContextService])
        ...
        BGTaskScheduler.shared.register(forTaskWithIdentifier: DiverApp.backgroundTaskIdentifier, using: nil) { task in
            DiverApp.handleAppRefresh(task: task as! BGAppRefreshTask, service: service)
        }

        // Generate and store a cryptographically secure random secret if it doesn't exist
        if keychainService.retrieveString(key: DiverApp.diverLinkSecretKey) == nil {
            let secret = Self.generateSecureSecret()
            do {
                try keychainService.store(key: DiverApp.diverLinkSecretKey, value: secret)
            ...
            do {
                ...
                if #available(iOS 16.0, macOS 13.0, *) {
                    ...
                    do {
                         ...
                         if let store = DiverApp.sharedDataStore {
    ...
    static func scheduleAppRefresh() {
        let request = BGAppRefreshTaskRequest(identifier: DiverApp.backgroundTaskIdentifier)
    ...
    private func handleWrappedLink(_ url: URL) {
        ...
        Task {
            do {
                // Get keychain secret
                guard let secretString = keychainService.retrieveString(key: DiverApp.diverLinkSecretKey),
