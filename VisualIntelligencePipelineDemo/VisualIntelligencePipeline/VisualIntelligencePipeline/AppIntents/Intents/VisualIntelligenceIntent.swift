...
/// Captures what's on screen using Visual Intelligence (OCR) and creates a Visual Intelligence link
struct VisualIntelligenceIntent: AppIntent {
    ...
    func perform() async throws -> some IntentResult & ReturnsValue<String> & ProvidesDialog & ShowsSnippetView {
        ...
        guard let secretString = keychainService.retrieveString(key: KeychainService.Keys.diverLinkSecret),
            ...
            return .result(
                value: "",
                dialog: "Link secret not found. Please set up the app first."
            )
        }

        // Wrap URL into Visual Intelligence link
        ...
        guard let queueDirectory = AppGroupContainer.queueDirectoryURL() else {
            return .result(
                value: wrappedString,
                dialog: "Created Visual Intelligence link but could not save to library: \(wrappedString)"
            )
        }
        ...
        if extractedURLs.count > 1 {
            baseMessage = "Found \(extractedURLs.count) URLs. Created Visual Intelligence link for: \(selectedURL.host ?? "link")\n\nLink copied to clipboard!"
        } else {
            baseMessage = "Created Visual Intelligence link for: \(selectedURL.host ?? "link")\n\nLink copied to clipboard!"
        }
        ...
        VStack(alignment: .leading, spacing: 12) {
            HStack(alignment: .top, spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    Text(foundCount > 1 ? "Found \(foundCount) Links" : "Scan Success")
                        .font(.headline)
