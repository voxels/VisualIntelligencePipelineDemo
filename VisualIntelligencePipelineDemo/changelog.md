...
### Mac App — AI Model Setup & Edge Node Polish

**MacInstallerView.swift — new first-launch installer**
- First-launch sheet shown before main UI; persisted via `vi.mac.installed` UserDefaults key.
- Downloads FastVLM 1.5B (in-app via MLX Swift, sandbox-safe) with live progress per step.
- CLaRa 7B and ML-Sharp shown as "Pending Edge Node" (delegated to EdgeDaemon, which runs unsandboxed).
- Done screen lists per-model status: FastVLM ✅ Ready, CLaRa/ML-Sharp 🟠 Pending Edge Node.
- "Skip" defers all provisioning to background; "Continue in Background" mid-install escape hatch.
- Correctly detects FastVLM already present (`isModelCached`) and skips re-download.

**MacSettingsView.swift — On-Device AI section expanded**
- Three sections: FastVLM 1.5B (toggle + download/delete), CLaRa 7B, ML-Sharp 3D Engine.
- CLaRa and ML-Sharp show file-system status (ready/not installed) via `homeDirectoryForCurrentUser` paths.
- "Provision via Edge Node" buttons trigger `EdgeModelProvisioner.downloadCLaRa/MLSharp()` in detached task.
- `ModelStatusRow` helper reacts to `modelStatusRefreshID` UUID token for live refresh.
- Fixed broken `desktopcomputer.slash` SF Symbol → `desktopcomputer.trianglebadge.exclamationmark`.

**EdgeNodeInstallService.swift — @Observable only + dev launch**
- Removed dual `@Observable`/`ObservableObject` conformance — was silently preventing `@Environment` views from updating.
- Dev builds (`isRunningFromDerivedData == true`) now search `~/Library/Developer/Xcode/DerivedData` for `VisualIntelligenceMacEdgeNode.app` and launch it directly via `NSWorkspace`, instead of showing an error.
- Fixed bundle ID: `com.secretatomics.visualintelligence.mac.edgenode` (matches `project.yml`).
- `MacAppCommands.body`: `requiresApproval` state now shows "Open System Settings…" directly in the menu instead of a disabled button; success/failure after `install()` shows `NSAlert`.

**VisualIntelligenceMacApp.swift**
- Switched `.environmentObject` → `.environment` to match `@Observable` injection model.
- `MacInstallerView` presented as blocking sheet on first launch; `EdgeNodeOnboardingView` fires after installer dismisses.

**MacContentView.swift**
- `@EnvironmentObject` → `@Environment(EdgeNodeInstallService.self)` to match `@Observable`.

**EdgeDaemonApp.swift — auto-provisioning on startup**
- `startListening()` + `EdgeModelProvisioner.shared.provisionAll()` now fire in EdgeDaemonMenu `.task`.
- EdgeDaemon is unsandboxed → Python/git subprocess provisioning for CLaRa/ML-Sharp works correctly here.

**EdgeModelProvisioner.swift**
- `provisionMLSharp()`: added explicit `terminationStatus == 0` check and `fileExists` guard after `git clone` before proceeding to venv setup.

**FastVLMEnrichmentService.swift**
- Removed `mlx-community/FastVLM-7B-bf16` from `optimalHuggingFaceRepo` — unsupported `llava_qwen2` architecture.
- HF Hub path now always `mlx-community/FastVLM-0.5B-bf16`; 1.5B provisioned locally via EdgeDaemon Application Support.

**project.yml (VisualIntelligenceMac)**
- Added `DiverUI` package + dependency (was missing post-regen, caused linker errors).
- Set `DEVELOPMENT_TEAM: 23264QUM9A` on both `VisualIntelligenceMac` and `VisualIntelligenceMacEdgeNode` targets.
- `VisualIntelligenceMacEdgeNode` bundle ID set to `com.secretatomics.visualintelligence.mac.edgenode`.
- `embed: true + codeSign: true` on EdgeNode target dependency produces correct Copy Files phase for Login Items.

**File changes:**
- `[NEW]` `VisualIntelligenceMac/Sources/Views/MacInstallerView.swift`
- `[MODIFIED]` `VisualIntelligenceMac/Sources/Views/MacSettingsView.swift`
- `[MODIFIED]` `VisualIntelligenceMac/Sources/Services/EdgeNodeInstallService.swift`
- `[MODIFIED]` `VisualIntelligenceMac/Sources/App/VisualIntelligenceMacApp.swift`
- `[MODIFIED]` `VisualIntelligenceMac/Sources/Views/MacContentView.swift`
- `[MODIFIED]` `VisualIntelligenceMac/project.yml`
- `[MODIFIED]` `EdgeDaemon/EdgeDaemonApp.swift`
- `[MODIFIED]` `DiverKit/Sources/DiverKit/Services/FastVLMEnrichmentService.swift`
- `[MODIFIED]` `DiverKit/Sources/DiverKit/Services/Provisioning/EdgeModelProvisioner.swift`


### Foursquare Integration Removed
- **Complete Removal**: Removed `FoursquareEnrichmentService.swift`, `FoursquareLinkEnrichmentService.swift`, and all references from the DI container, MapKit search aggregator, pipeline components, UI views, and tests. Location enrichment now exclusively relies on Apple MapKit and Contacts.
- `[REMOVE]` `DiverKit/Sources/DiverKit/Services/FoursquareEnrichmentService.swift`
- `[REMOVE]` `VisualIntelligencePipeline/VisualIntelligencePipeline/Services/FoursquareLinkEnrichmentService.swift`

### Mac App — Full iPad Parity

**DiverUI view consolidation**
- Removed 22 iOS-specific duplicate view files from the `VisualIntelligencePipeline` target. `ProductProfileView`, `DocumentProfileView`, `ImageProfileView`, `PlaceProfileView`, `PersonProfileView`, `QRCodeProfileView`, `WebLinkProfileView`, and 15 Commerce/Library/Session views are now sourced exclusively from the `DiverUI` package.
- Added `import DiverUI` to `ReferenceDetailView.swift`, `ContentView.swift`, `SettingsView.swift`, and `SidebarView.swift` to resolve DiverUI types.
- Fixed `EdgeNodeSetupView` compile error: `connect(to:)` is throwing — added `try?`.

**MacContentView.swift — complete rewrite**
- Sidebar now matches iOS `SidebarView` 1:1: CLaRa chat entry point, `DailySummaryCard`, Favorites section, Collections with `DisclosureGroup`, standalone Sessions, context menus (rename, delete, edit location, analyze), queue progress overlay, full maintenance state.
- Detail pane replaced with full DiverUI profile stack: `ProductProfileView`, `ImageProfileView`, `WebLinkProfileView`, `DocumentProfileView`, `PlaceProfileView`, `QRCodeProfileView`, `PersonProfileView`, header (image/title/tags/status/URL), footer (purposes, VLM insights, media info, `ConceptWeightingSection`).
- Drop ingestion wired to pipeline: `handleDroppedImages`/`handleDroppedURLs`/`handleDroppedFiles` now insert `LocalInput` into a background `ModelContext` and call `processPendingQueue()`.
- Continuity Camera button in toolbar grouped under a camera menu (with FaceTime/USB fallback and warning label).
- Item list context menus: Favorite, Reprocess, Delete.
- Settings sheet (`MacSettingsView`) and Reprocessing sheet (`MacReprocessingView`) wired to toolbar maintenance menu.

**MacSettingsView.swift — new**
- `NavigationSplitView` settings sheet mirroring iOS `SettingsView`.
- Sections: General (Rebuild Library, Reprocess Pipeline, Shortcut Gallery), Commerce (EthicalPolicyConfigView, OwnedProductsView, APIKeyConfigView), Edge Node (EdgeNodeSetupView from DiverUI), On-Device AI (FastVLM download/toggle/delete), Storage (item count, Delete Database, Export Logs → NSWorkspace reveal), About (version/build).

**MacReprocessingView.swift — new**
- 4-step reprocessing wizard matching iOS `ReprocessingWizardView` (Config → Processing → Review → Complete).
- Date-picker cutoff, live progress bar, per-item approve/reject review step, snapshot rollback on reject.

- `[REWRITE]` `VisualIntelligenceMac/Sources/Views/MacContentView.swift`
- `[NEW]` `VisualIntelligenceMac/Sources/Views/MacSettingsView.swift`
- `[NEW]` `VisualIntelligenceMac/Sources/Views/MacReprocessingView.swift`
- `[MODIFY]` `VisualIntelligencePipeline/View/ReferenceDetailView.swift` — import DiverUI
- `[MODIFY]` `VisualIntelligencePipeline/View/ContentView.swift` — import DiverUI
- `[MODIFY]` `VisualIntelligencePipeline/View/SettingsView.swift` — import DiverUI
- `[MODIFY]` `VisualIntelligencePipeline/View/SidebarView.swift` — import DiverUI
- `[REMOVE]` 22 iOS duplicate view files moved to trash (ProductProfileView, DocumentProfileView, ImageProfileView, PlaceProfileView, PersonProfileView, QRCodeProfileView, WebLinkProfileView, AgenticChatView, APIKeyConfigView, CommerceActionView, EthicalPolicyConfigView, NowcastChartView, OwnedProductsView, OwnershipButton, ProductScoreOverlayView, ScoreHistoryChartView, ConceptListView, ConceptWeightingSection, QueueProgressView)
- `[MODIFY]` `DiverUI/Sources/DiverUI/Settings/EdgeNodeSetupView.swift` — add try? to connect(to:) call
