...
*   **Open Product Database Cascade:** `ESGEnrichmentService` queries 4 free Open *Facts databases (Food → Beauty → Pet Food → Products Facts) then falls back to **UPC Item DB** (upcitemdb.com, free tier, no API key) for non-food UPC/EAN barcodes. All with 24h cache. Returns product name, brand, 12+ text fields (ingredients, allergens, NOVA processing, nutrition, packaging, origins) plus numerical scores.
...
*   **`MetadataPipelineService`:** Converts queue items to `LocalInput` and orchestrates metadata extraction. Uses `Task(priority: .utility)` for all heavy processing. After batch Phase 1, calls `enrichCapturedItems()` to sweep `.captured` items through Phase 2 in background.
...
*   **Reprocessing:** All reprocessing flows through a single code path: `processItemByID` (private `ModelContext` per call). Each call clears calculated data (summary, transcription, tags, purposes, categories, questions, web/document/QR/activity context) while preserving user-set location data (detected via `placeContext.contactIdentifier` or placeID prefixes `mapkit-`, `mk-`, `home-location`). The full enrichment pipeline runs: Link Enrichment → EXIF/GPS Extraction → Visual Analysis (7 Vision requests) → Reverse Geocoding → Parallel Enrichment → Edge CLaRa/SLM → FastVLM → Commerce Scoring → Concept Extraction → CLaRa RAG Ingestion. **Freshness guard**: `processItemByID` re-fetches the item's status at start; if already `.ready` or `.processing` (e.g., processed by another device via CloudKit sync), it skips processing. `force: true` bypasses this guard. Entry points:
    *   `processItemImmediately`: Thin wrapper — cancels queue, delegates to `processItemByID(force: true)`, restarts queue. Used by "Process Now" button.
    *   `reprocessPipeline` (Settings > Reprocess Pipeline): **Lifecycle-safe 3-phase design**: Phase 1 durably marks all matching items as `.queued` in SwiftData (clears calculated data, clears parent session summaries, saves checkpoint). Phase 2 processes each item sequentially via `processItemByID`. Phase 3 regenerates session summaries for all affected sessions via `generateAndSaveSessionSummary`. If the app is killed during Phase 2, the next foreground resume picks up remaining `.queued` items via `processQueuedOrphanItems`.
    *   `reprocessSession` (sidebar context menu): Collects item IDs for the session, processes each via `processItemByID` in a `Task`, then regenerates the session summary.
    *   `processQueuedOrphanItems` (lifecycle resume): Automatically picks up `.queued` items that have no corresponding `LocalInput` (e.g., from interrupted reprocessing) and processes them via `processItemByID`.
...
*   `Services/Edge/NWTransportLayer.swift` — TLS 1.3, ConnectionSerializer actor, length-prefixed framing, self-healing connections
...
7.  **Main Thread Safety & Concurrency (Swift 6):**
    *   **Avoid `Task` anti-pattern:** Breaking task hierarchy with `Task` causes priority inversion and breaks cancellation propagation. Use structured concurrency (`Task { }`) instead.
    *   **Use Stored, Cancellable Tasks:** In view-models or long-running services, retain task handles (e.g. `var currentTask: Task<Void, Never>?`) and explicitly cancel them when appropriate. Check `Task.checkCancellation()` often inside loops or heavy work instead of using uncontrollable detached tasks.
    *   **Actor-Based Backgrounding:** To move work off the `@MainActor`, extract the heavy logic into an `actor` or use `nonisolated async` methods on the `@MainActor` class. Simple `Task { }` blocks will then naturally execute this work on the global concurrent pool without blocking the main thread.
    *   Keep long-running pipeline tasks off the main thread using these actor patterns.
    *   **Never use `Task { }` directly containing heavy synchronous work on the main actor** — always wrap the work in a `nonisolated` function first.
    ...
    *   **Use `processItemByID`** for all reprocessing. `processItemImmediately` is a thin wrapper (cancels queue → delegates to `processItemByID` → restarts queue). All reprocessing logic lives in one canonical code path with a private `ModelContext` per call.
    *   **Never spawn N concurrent Tasks in a for-loop** that each mutate SwiftData models. Collect IDs first, then process sequentially in a single `Task`.
...
*   **Unified reprocessing:** `processItemByID` is the single canonical reprocessing path. `processItemImmediately` delegates to it after cancelling the queue. Architecture tests enforce that ViewModels never call `processItemImmediately` directly.
...
*   **`Services.shared`:** Global singleton accessed from `@MainActor` context. Accesses from `Task` must use `await MainActor.run { Services.shared.someService }`.
...
*   **SE-0461 Isolation regions:** Defines sending rules between nonisolated, actor-isolated, and `@concurrent` contexts. Governs how values cross isolation boundaries in `Task` closures.
