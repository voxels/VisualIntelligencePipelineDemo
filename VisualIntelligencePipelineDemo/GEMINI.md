...
*   **`LocalPipelineService`:** The core orchestrator. Handles ingestion, enrichment, and persistence. Implements **edge-first intelligence routing**: checks for edge CLaRa 7B before running SLM (Stage 1). If edge available, calls `EdgeContextActor.summarizeStructured()` for structured output (summary + tags + statements + purpose), skipping SLM and local FastVLM.
...
*   **Reprocessing:** Supports silent background reprocessing via `processItemByID` (private `ModelContext` per call) and bulk `reprocessPipeline` to update metadata. `processItemByID` runs the full pipeline: Vision → LLM → FastVLM → CLaRa ingestion. Reuses existing item IDs to prevent duplicates.
