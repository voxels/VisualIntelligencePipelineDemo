...
*   **Reprocessing:** All reprocessing flows through a single code path: `processItemByID` (private `ModelContext` per call). `processItemImmediately` is a thin wrapper that cancels the queue, delegates to `processItemByID`, then restarts the queue. Runs both phases (full pipeline: Vision → LLM → FastVLM → Commerce → CLaRa ingestion). Reuses existing item IDs to prevent duplicates.
